...
// --- Sidebar Component ---
const Sidebar = ({ isOpen, onClose, onNavigate, onLogout, unreadCount }) => {
        ...
        <div className="fixed inset-0 z-50">
            ...
            <div className="relative w-72 h-full bg-rose-50 shadow-xl p-6 flex flex-col">
                ...
                <nav className="flex flex-col gap-4">
                    ...
                    <button onClick={() => onNavigate('allChats')} className="relative text-left text-lg text-slate-700 hover:text-rose-500 p-2 rounded-lg hover:bg-rose-100">
                        My Chats
                        {unreadCount > 0 && (
                    ...
                    <button onClick={() => onNavigate('connects')} className="text-left text-lg text-slate-700 hover:text-rose-500 p-2 rounded-lg hover:bg-rose-100">Connects</button>
                    <button onClick={() => onNavigate('confessions')} className="text-left text-lg text-slate-700 hover:text-rose-500 p-2 rounded-lg hover:bg-rose-100">Confessions</button>
...
const DashboardPage = ({ students, onViewProfile, onStartChat, onMenuClick, currentUser, unreadCount }) => (
    <div className="min-h-screen p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
                <button onClick={onMenuClick} className="relative p-2 rounded-full hover:bg-rose-100 text-slate-600 hover:text-rose-500">
                    <MenuIcon className="w-8 h-8"/>
                    {unreadCount > 0 && (
                        <span className="absolute top-0 right-0 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-xs font-bold text-white">
...
const FullProfilePage = ({ profileId, onBack, onStartChat }) => {
    ...
    const handleSendRequest = async () => {
        try {
            const res = await axios.post('/connections/request', { receiverId: profileId });
            setConnectionStatus(res.data.status);
        } catch (err) {
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg p-8">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-6">
                        ...
                        ) : connectionStatus === 'awaiting-acceptance' ? (
                            <button onClick={handleSendRequest} className="w-full bg-green-500 text-white font-bold py-3 px-6 rounded-xl hover:bg-green-600 transition-all">
                                Accept Request
                            </button>
                        ) : (
...
// --- Connects Page Component ---
const ConnectsPage = ({ onBack, onViewProfile }) => {
    ...
    const handleAcceptRequest = async (senderId) => {
        try {
            await axios.post('/connections/accept', { senderId });
            fetchRequests(); 
        } catch (err) {
        ...
        <div className="min-h-screen p-4 sm:p-6 md:p-8">
            <div className="max-w-2xl mx-auto">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h1 className="text-4xl font-bold text-slate-800" style={{ fontFamily: "'Playfair Display', serif" }}>Connections</h1>
                </div>
                <div className="mb-12">
                    <h2 className="text-2xl font-bold text-slate-700 mb-4 border-b pb-2">Requests Received</h2>
                    {pendingRequests.length > 0 ? (
                        <div className="space-y-4">
                            {pendingRequests.map(user => (
                                <div key={user._id} className="flex items-center justify-between p-4 bg-white rounded-xl shadow-md">
                                    <div onClick={() => onViewProfile(user)} className="flex items-center cursor-pointer">
                                        <img src={user.profilePic || 'https://placehold.co/100x100/cccccc/ffffff?text=Pic'} alt={user.name} className="w-16 h-16 rounded-full object-cover" />
                                        <div className="ml-4">
                                            <h3 className="text-lg font-bold text-slate-800">{user.name}</h3>
                                            <p className="text-slate-500 text-sm">{user.course}</p>
                                        </div>
                                    </div>
                                    <button onClick={() => handleAcceptRequest(user._id)} className="bg-green-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-green-600 transition-colors">
                                        Accept
                                    </button>
                                </div>
                            ))}
                        ...
                        <p className="text-slate-500">You have no pending connection requests.</p>
                    )}
                </div>
                <div>
                    <h2 className="text-2xl font-bold text-slate-700 mb-4 border-b pb-2">Requests Sent</h2>
                    {sentRequests.length > 0 ? (
                        <div className="space-y-4">
                            {sentRequests.map(user => (
                                <div key={user._id} onClick={() => onViewProfile(user)} className="flex items-center p-4 bg-white rounded-xl shadow-md cursor-pointer">
                                    <img src={user.profilePic || 'https://placehold.co/100x100/cccccc/ffffff?text=Pic'} alt={user.name} className="w-16 h-16 rounded-full object-cover" />
                                    <div className="ml-4">
                                        <h3 className="text-lg font-bold text-slate-800">{user.name}</h3>
                                        <p className="text-slate-500 text-sm">Request pending</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <p className="text-slate-500">You haven't sent any connection requests.</p>
                    )}
                </div>
            </div>
        </div>
...
// --- Confessions Page Component ---
const ConfessionsPage = ({ onBack }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post('/confessions', { text: newConfession });
            setConfessions([res.data, ...confessions]);
            setNewConfession('');
        } catch (err) {
            console.error("Failed to post confession", err);
            alert("Could not post confession. Please try again.");
        }
    };
        ...
        <div className="min-h-screen p-4 sm:p-6 md:p-8">
            <div className="max-w-2xl mx-auto">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h1 className="text-4xl font-bold text-slate-800" style={{ fontFamily: "'Playfair Display', serif" }}>Confessions</h1>
                </div>
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 mb-8">
                    <form onSubmit={handleSubmit}>
                        <textarea
                            ...
                            maxLength="500"
                            className="w-full p-3 border border-slate-300 rounded-lg h-28 resize-none focus:outline-none focus:ring-2 focus:ring-rose-300"
                        />
                        <button type="submit" className="w-full mt-4 bg-rose-400 text-white font-bold py-3 px-6 rounded-xl hover:bg-rose-500 transition-all">
                            Post Anonymously
                        </button>
                    </form>
                </div>
                <div className="space-y-4">
                    {confessions.map(confession => (
                        <div key={confession._id} className="bg-white p-5 rounded-xl shadow-md">
                            <div className="flex items-center mb-3">
                                <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-500">?</div>
                                <div className="ml-3">
                                    <h3 className="font-bold text-slate-800">Anonymous</h3>
                                    <p className="text-xs text-slate-400">
                                        {formatDistanceToNow(new Date(confession.createdAt), { addSuffix: true })}
                                    </p>
                                </div>
                            </div>
                            <p className="text-slate-700 leading-relaxed">{confession.text}</p>
                        </div>
                    ))}
                </div>
            </div>
...
export default function App() {
  ...
  const handleSidebarNavigate = (page) => {
    // --- FIX ---: Restore the correct logic for navigating to the edit profile page
    if (page === 'editProfile') {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'chat':
        return <ChatPage key={selectedStudent._id} student={selectedStudent} currentUser={currentUser} onBack={() => setCurrentPage('dashboard')} onChatOpened={fetchUnreadCount} />;
      case 'allChats':
        return <AllChatsPage chatPartners={conversations} onChat={handleStartChat} onBack={handleBackToDashboard} />;
      case 'connects':
        return <ConnectsPage onBack={() => setCurrentPage('dashboard')} onViewProfile={handleViewProfile} />;
      case 'confessions':
        return <ConfessionsPage onBack={() => setCurrentPage('dashboard')} />;
      case 'dashboard':
        return <DashboardPage students={students} currentUser={currentUser} onViewProfile={handleViewProfile} onStartChat={handleStartChat} onMenuClick={() => setIsSidebarOpen(true)} unreadCount={unreadCount} />;
      case 'fullProfile':
...
// // --- Full Profile Page Component (FIXED) ---
// const FullProfilePage = ({ profileId, onBack, onStartChat }) => {
//     const [profileData, setProfileData] = useState(null);
//     const [connectionStatus, setConnectionStatus] = useState('none');
//     const [isLoading, setIsLoading] = useState(true);
...
//         try {
//             const res = await axios.post('/connections/request', { receiverId: profileId });
//             // Update status based on the server's response
//             setConnectionStatus(res.data.status); 
//         } catch (err) {
//             console.error("Failed to send request", err);
//         }
...
//                 <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg p-8">
//                     {/* --- FIX ---: Restored the missing profile details UI */}
//                     <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
//                         <img src={profileData.profilePic || 'https://placehold.co/200x200/cccccc/ffffff?text=Pic'} alt={profileData.name} className="w-32 h-32 rounded-full object-cover border-4 border-white shadow-md" />
//                         <div className="text-center sm:text-left">
//                             <h1 className="text-4xl font-bold text-slate-800" style={{ fontFamily: "'Playfair Display', serif" }}>{profileData.name}</h1>
...
//                     <div className="flex flex-col sm:flex-row gap-4 mt-6">
//                         {/* GChat button is always visible but disabled if not connected */}
//                         <button 
//                             onClick={() => onStartChat(profileData)} 
//                             className="w-full bg-violet-500 text-white font-bold py-3 px-6 rounded-xl hover:bg-violet-600 transition-all flex items-center justify-center gap-2 disabled:bg-slate-300 disabled:cursor-not-allowed"
...
//                 </button>
//                 <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg p-8">
//                     {/* ... (profile details like image and name are unchanged) */}
...
//                                 Request Sent
//                             </button>
//                         // --- NEW ---: Handle the case where the other user sent a request to you
//                         ) : connectionStatus === 'awaiting-acceptance' ? (
//                             <button onClick={handleSendRequest} className="w-full bg-green-500 text-white font-bold py-3 px-6 rounded-xl hover:bg-green-600 transition-all">
//                                 Accept Request
//                             </button>
